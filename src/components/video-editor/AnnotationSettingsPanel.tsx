...
export function AnnotationSettingsPanel({
	...
	const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!validTypes.includes(file.type)) {
			toast.error(t("settingsToast.invalidFileType"), {
				description: t("settingsToast.pleaseUploadImage"),
			});
			event.target.value = "";
			return;
		}
		...
		reader.onload = (e) => {
			...
			if (dataUrl) {
				...
				toast.success(t("settingsToast.imageUploadedSuccess"));
			}
		};

		reader.onerror = () => {
			toast.error(t("settingsToast.failedToUploadImage"), {
				description: t("settingsToast.errorReadingFile"),
			});
		};
		...
		<div className="flex-[2] min-w-0 bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col shadow-xl h-full overflow-y-auto custom-scrollbar">
			<div className="mb-6">
				<div className="flex items-center justify-between mb-4">
					<span className="text-sm font-medium text-slate-200">{t("annotation.settings")}</span>
				...
				>
					...
					<TabsContent value="text" className="mt-0 space-y-4">
						<div>
							<label className="text-xs font-medium text-slate-200 mb-2 block">{t("annotation.textContent")}</label>
							<textarea
						...
						<div className="space-y-4">
							...
							<div className="grid grid-cols-2 gap-2">
								...
								<div>
									<label className="text-xs font-medium text-slate-200 mb-2 block">{t("annotation.size")}</label>
									<Select
									...
									<Popover>
										<PopoverTrigger asChild>
											...
											>
												...
												<span className="text-xs text-slate-300 truncate flex-1 text-left">
													{annotation.style.backgroundColor === "transparent" ? t("annotation.none") : t("annotation.colorLabel")}
												</span>
												<ChevronDown className="h-3 w-3 opacity-50" />
											</Button>
										</PopoverTrigger>
						...
						{annotation.content && annotation.content.startsWith("data:image") && (
							<div className="rounded-lg border border-white/10 overflow-hidden bg-white/5 p-2">
								<img
									...
									alt={t("annotation.uploadedAnnotation")}
					...
					<TabsContent value="figure" className="mt-0 space-y-4">
						...
						<div>
							<label className="text-xs font-medium text-slate-200 mb-2 block">
								{t("annotation.strokeWidth")}: {annotation.figureData?.strokeWidth || 4}px
							</label>
							<Slider
						...
						<div>
							<label className="text-xs font-medium text-slate-200 mb-2 block">{t("annotation.arrowColor")}</label>
							<Popover>
								<PopoverTrigger asChild>
									<Button
										variant="outline"
				...
				<div className="mt-6 p-3 bg-white/5 rounded-lg border border-white/5">
					<div className="flex items-center gap-2 mb-2 text-slate-300">
						...
						<span className="text-xs font-medium">{t("annotation.shortcutsTips")}</span>
					</div>
					<ul className="text-[10px] text-slate-400 space-y-1.5 list-disc pl-3 leading-relaxed">
						<li>{t("annotation.tipMovePlayhead")}</li>
						<li>{t("annotation.tipTabCycle", { key: "Tab" })}</li>
						<li>{t("annotation.tipShiftTabCycle", { key: "Shift+Tab" })}</li>
					</ul>
				</div>
			</div>
		</div>
