...
export function ExportDialog({
	...
	const getStatusMessage = () => {
		...
		if (isCompiling || isFinalizing) {
			if (renderProgress !== undefined && renderProgress > 0) {
				return t("export.compilingGifProgress", { progress: renderProgress });
			}
	...
	const getTitle = () => {
		...
		return exportFormat === "gif" ? t("export.exportingGif") : t("export.exportingVideo");
	};
			...
			<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] bg-[#09090b] rounded-2xl shadow-2xl border border-white/10 p-8 w-[90vw] max-w-md animate-in zoom-in-95 duration-200">
							...
							<>
								...
								<div className="flex flex-col gap-2">
									...
									<span className="text-sm text-slate-400">
										{t("export.videoReady", { label: formatLabel.toLowerCase() })}
									</span>
									{exportedFilePath && (
										<Button
											variant="secondary"
					...
					<div className="space-y-6">
						<div className="space-y-2">
							<div className="flex justify-between text-xs font-medium text-slate-400 uppercase tracking-wider">
								<span>{isCompiling || isFinalizing ? t("export.compilingGif") : t("export.renderingFrames")}</span>
						...
						<div className="grid grid-cols-2 gap-4">
							<div className="bg-white/5 rounded-xl p-3 border border-white/5">
								...
								<div className="text-slate-200 font-medium text-sm">
									{isCompiling || isFinalizing ? t("export.compiling") : formatLabel}
								</div>
							</div>
							<div className="bg-white/5 rounded-xl p-3 border border-white/5">
								<div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">
				...
				{showSuccess && (
					<div className="text-center py-4 animate-in zoom-in-95">
						<p className="text-lg text-slate-200 font-medium">{t("export.videoSavedSuccess", { label: formatLabel })}</p>
					</div>
				)}
			</div>
		</>
