			...
			<DialogContent className="max-w-2xl bg-[#09090b] border-white/10 [&>button]:text-slate-400 [&>button:hover]:text-white">
				<DialogHeader>
					<DialogTitle className="text-xl font-semibold text-slate-200 flex items-center gap-2">
						<Scissors className="w-5 h-5 text-[#ef4444]" /> {t("tutorial.howTrimmingWorksTitle")}
					</DialogTitle>
					<DialogDescription className="text-slate-400">
						{t("tutorial.howTrimmingWorksDesc")}
					</DialogDescription>
				</DialogHeader>
				<div className="mt-4 space-y-8">
					...
					<div className="space-y-2">
						...
						<div className="relative h-24 bg-[#000] rounded-lg border border-white/10 flex items-center px-4 overflow-hidden select-none">
							...
							<div className="absolute left-[5%] text-[10px] text-slate-400 font-medium">{t("tutorial.kept")}</div>
							<div className="absolute left-[50%] text-[10px] text-slate-400 font-medium">{t("tutorial.kept")}</div>
							<div className="absolute left-[90%] text-[10px] text-slate-400 font-medium">{t("tutorial.kept")}</div>
						</div>
						...
						<div className="relative h-12 bg-[#000] rounded-lg border border-white/10 flex items-center justify-center gap-1 px-4 select-none">
							<div
								...
								style={{ width: "30%" }}
							>
								<span className="text-[10px] text-white font-medium">{t("tutorial.part", { num: 1 })}</span>
							</div>
							<div
								className="h-8 bg-slate-700 rounded flex items-center justify-center opacity-80"
								style={{ width: "30%" }}
							>
								<span className="text-[10px] text-white font-medium">{t("tutorial.part", { num: 2 })}</span>
							</div>
							<div
								className="h-8 bg-slate-700 rounded flex items-center justify-center opacity-80"
								style={{ width: "30%" }}
							>
								<span className="text-[10px] text-white font-medium">{t("tutorial.part", { num: 3 })}</span>
							</div>
							<span className="absolute right-4 text-xs text-slate-400">{t("tutorial.finalVideo")}</span>
						</div>
					</div>
					{/* Steps */}
